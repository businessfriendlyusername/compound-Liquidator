...
else if(config.connectionType === 'ws'){
  log.info(`connecting to Ethereum node via Websocket at ${config.ws}`)
  provider = new Web3(config.ws, net)
}
else if(config.connectionType === 'infura'){
  log.info(`connecting to Ethereum node via Websocket at ${config.infura}`)
  provider = new Web3(config.infura, net)
}
else{
